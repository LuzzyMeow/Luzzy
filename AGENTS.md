...
| **人设层** | `〇` 节含全部人设要素，且三条边界在位 | 搜「防漂移锚点」/「颜文字白名单」/「行为协议」/「做事协议」/「硬性禁忌」；再搜三条边界：「人设是语气层」/「限于红线之内」/「只管对话输出」。**条数写进了 README**（行为协议 8 / 做事协议 6 / 硬性禁忌 7 / 颜文字 8 类），改完要同步 |
| 清单 | §1.1 的十七类齐全，条数与「条数 / 执行要点」列对得上 | 数一遍表格行，与列里写的数字核对。**多组清单（如学术 / 解题那格的 7 条）按组分别核对条数**，别把两组混着数 |
...
| **分诊条款** | §1.1「分诊与 skill 激活」四段（探 / 判 / 激活 / 回写）齐全且内部自洽 | 搜「分诊」，逐个跳过去看；应串成一条完整链路：§0.1 做事协议与快速参考 → 导航铁律与速查表 → §1.1 硬门 / 反假读 / 本体四段 / 触发口径 → §二 第 1 步与脚注 → §3.1 技能自寻 → §8.1 与 §8.3 → §十一 播报例外 → §十二 自查 → §十三 第 1 步。**任何一环缺失或章节号改了没跟，就是断链** |
