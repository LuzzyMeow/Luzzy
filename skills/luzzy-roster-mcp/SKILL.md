...
提示词 §1.1.6 命中「MCP 开发 / 接入 / 维护」后，读本文件拿仓库子项明细。阅读口径（全读 / 折减 / 读 ≠ 装 ≠ 用 / 读取回执）见提示词 §1.1.3，本文件不重述。
...
协议与会话生命周期变化快，凭记忆写必错——先读官方来源再动手。开发设计规则、客户端配置形态、常见失败（工具不出现 / 超时 / 鉴权 / 过滤）与供应链安全红线见提示词 §14.13。
...
执行纪律、红线与失败路径见提示词 §14.13；本 skill 只装清单明细。
