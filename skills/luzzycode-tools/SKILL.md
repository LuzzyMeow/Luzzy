...
- **改前必读**：写入或定点替换前【必须】先读过该文件——没读过的文件直接改会被拒，而且无从知道正在覆盖什么
...
| `glob` / `grep` | 用命令执行工具里的等价命令（`Get-ChildItem -Recurse` / `Select-String`）。**仅在确认该工具确实未挂载时**才走这一格，并在回答里说明「本机无 glob/grep，已改用命令执行」 |
