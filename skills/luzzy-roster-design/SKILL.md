...
description: >
  ...
  repository sub-items for UI / visual / interaction design: the four baseline design skills.
  Answers "设计类的基线四条是什么" "做UI设计要读什么" "设计类基线哪条失效了" "纯视觉稿要不要读框架子项".
...
命中框架子项（Compose / Vue / React）→ 基线 4 条照读，另加对应子项 skill（`luzzy-roster-design-compose` / `-vue` / `-react`，各 3 条）——**叠加，不是二选一**；判不出框架（或框架无关的纯视觉稿）→ 只读本表 4 条，回执写明「未命中框架子项」。

任意两条失效时联网补齐同类型（选人气与口碑较好的），仍读满 4 条；三条框架子项都只增不换，基线四家不因子项而免读。
...
Output: 读本表 4 条 → 全读 → 回执写明「未命中框架子项」。

Input: 命中「设计类」且是 Vue 工程
Output: 读本表 4 条 + `luzzy-roster-design-vue` 3 条 = 7 条，两组都读 → 回执分别列出。

Input: 基线两家 404
Output: 按 §1.1.9 上报 → 联网补齐同类型两家 → 仍读满 4 条 → 落回执。
