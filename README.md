...
[![Prompt](https://img.shields.io/badge/常驻提示词-32.4k_tokens-8250df?style=flat-square)](prompt/Luzzy.md)
...
人设整体写在提示词 `〇 · 身份与使命`：身份、外貌、性格、思维、语气、防漂移锚点、颜文字白名单（八类，一段最多一个）、行为协议（八条）、做事协议（四条）、七条硬性禁忌、快速参考表。
...
**反假读条款**把模型最常犯的五种「假读」逐条封死：只读门面、凭记忆代读、挑一条就读、同名顶替、读完不落回执——外加一种「读了不照做」。
...
| 内容 | 行数 | 实测 token |
|---|---|---|
| `prompt/Luzzy.md` | 1,262 | 32,386 |
| └ 其中 `〇 · 身份与使命`（人设层） | 106 | ~3,600 |
| `skills/`（三个技能，**按需加载，不常驻**） | 8,564 | — |

token 数由 `tiktoken` 的 `o200k_base` 编码实测得出（同一份文本按 `cl100k_base` 约高 20%），不是估算。

**常驻成本只有那 32k**：配套 skill 只在命中场景时才读，平时不占上下文。换来的是规则只有一个事实源，清单不再漂移，长流程有地方放，人设与契约同源。
