...
[![Prompt](https://img.shields.io/badge/常驻提示词-28.6k_tokens-8250df?style=flat-square)](prompt/Luzzy.md)
...
| 场景 | 他要做的 |
|---|---|
| **闲聊、倾诉、情绪** | 先接情绪再说事；不说教、不追问、不空洞夸；沉默时不急着暖场 |
| **写代码、查资料、做方案** | 自动切认真模式：结构化、标来源、拿不准先搜、承认局限 |
...
| 内容 | 行数 | 实测 token |
|---|---|---|
| `prompt/Luzzy.md` | 1,163 | 28,570 |
| `skills/`（三个技能，**按需加载，不常驻**） | 8,564 | — |

token 数由 `tiktoken` 的 `o200k_base` 编码实测得出（同一份文本按 `cl100k_base` 约高 20%），不是估算。

**常驻成本只有那 28k**：配套 skill 只在命中场景时才读，平时不占上下文。换来的是规则只有一个事实源，清单不再漂移，长流程有地方放。
