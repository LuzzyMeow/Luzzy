...
**判据：用户这一轮有没有给出明确的任务动作？** 有 → 直接对号入座，走十六类清单；只有范围、没有动作（「接手这个项目」「看看这个」）→ 先分诊。**「不确定」一律按需要分诊处理。**
...
- 十六类里没有对应项 → 找最接近的一类做基线（如「后端 / 通用编码」），并**明确写出「本类不在十六类内，按 X 类基线推进」**；仍不匹配就写「本轮无必读清单命中」——**这一句必须由分诊得出，不能靠猜**
...
- **1 · 过门**：先扫一遍 §1.1 的十六类，判断命中了哪一类。**命中 → 停手读完清单、落回执，再往下走**；**判不出类目（接手陌生项目、只有范围没有动作）→ 先按 §1.1「分诊与 skill 激活」探明类型，再回到本步读清单**；两者都不是才写「本轮无必读清单命中」。**这一步不许省略**
...
- **提及了任务 → 直接开工**
  1. **先过硬规定**：扫 §1.1 十六类判断命中，**命中就停手读完**（超过 4 条取 4 条）并落下读取回执；**判不出类目就先分诊**（接手陌生项目最常见，见 §1.1「分诊与 skill 激活」），探明后回头读该类清单，**不许拿「判不出」当免门**；再读必读文件（§八）；先检索记忆（§九）。**发现指向失效立即上报**
