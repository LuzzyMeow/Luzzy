...
- **任务类型不明的时候，先探明白再动手。** 接手一个陌生项目、对方只丢来一个目录、或者这事说不清属于哪一类——别急着改第一行代码，也别假装自己知道。先读、先判，把类型落到纸面上，再按那一类的方法干活（怎么判见 §1.1 分诊）。**「不确定」是探索的信号，不是跳过规则的借口。**
- **有 skill 就用 skill。** 每轮开工前扫一眼本机给了哪些 skill、项目自己带了哪些（§3.1）。有对口的就加载正文照它做，别凭手感硬来——清单是**必读的下限**，skill 是**做完的捷径**。读清单是补课，用 skill 是本事。
...
| 类型不明 | 先探、再判、后动手——「不确定」是探索的信号，不是跳过规则的借口 |
| 有对口 skill | 按名加载正文照它做——清单是下限，skill 是捷径 |
...
| **判不出类目先分诊** | §1.1 | 接手陌生项目＝类型未知；探明类型 → 读该类清单 → 扫本机与项目 skill；「判不出」不是免门 |
...
| 我要…… | 去哪 |
|---|---|
| 判断这个任务要不要先读东西 | §1.1 必读清单（十七类） |
| 分不清这项目属于哪一类 | §1.1 分诊与 skill 激活（三行分诊 + 激活清单） |
...
- **识别不出类目 ≠ 没有清单**：接手陌生项目、任务描述里没有任何一类线索时，先按下面的「分诊与 skill 激活」探出类目，再回到本门——**判不出类目不是跳过门的理由**
...
- 按 §1.1 读完该类的清单，落下读取回执——**分诊不是终点，是入口**
...
| **学术研究 / 论文撰写 / 学科题目解答** | **学术与论文**（做研究 / 读文献 / 写论文时读这 4 条）：<br>① **academic-research-skills** `https://github.com/Imbad0202/academic-research-skills`<br>② **Supervisor-Skills** `https://github.com/HKUSTDial/Supervisor-Skills`<br>③ **scientific-agent-skills** `https://github.com/K-Dense-AI/scientific-agent-skills`<br>④ **AI-Research-SKILLs** `https://github.com/Orchestra-Research/AI-Research-SKILLs`<br>**解题方法论**（做题 / 验算 / 核查解答时读这 3 条）：<br>⑤ Anthropic 官方 **math-olympiad** `https://github.com/anthropics/claude-plugins-official/tree/main/plugins/math-olympiad`<br>⑥ **求是Skill** `https://github.com/HughYau/qiushi-skill`<br>⑦ **cc-thinking-skills** `https://github.com/tjboudreaux/cc-thinking-skills` | **两组各自按「4 条及以内 → 全读」走**——学术 / 论文读 ①–④，题目解答读 ⑤–⑦；**⑤–⑦ 读的是「怎么想、怎么验」的方法论，不是科目知识点**；③ 体积 252 MB、① 与 ② 许可为 NOASSERTION——**读 ≠ 装**，装之前先核体积与许可 |
...
- **「类目未知」本身就算命中**：接手陌生项目、只给范围不给动作、任务描述与哪一类都对不上——**一律先分诊**（见上），探出类目后按该类的清单读。**不许因为「对不上关键词」就写成「本轮无命中」**
...
- **「学术研究 / 论文撰写」**：查文献、综述、读论文、写论文、投稿、审稿、复现实验、做研究计划——**读 ①–④**
- **「学科题目解答 / 解题方法论」**：做题、讲题、验算、核查某个解答对不对、要证明或反例、想找通用的解题方法——**读 ⑤–⑦**。注意这一格读的是**「怎么想、怎么验」的方法论**（审题解读、分步求解、对抗式验证、敢弃答），**不是科目知识点**；数学、物理、代码、逻辑、业务问题都走同一套方法，`math-olympiad` 的竞赛外壳只是它的压力测试场
...
- **1 · 过门**：先扫一遍 §1.1 的十七类，判断命中了哪一类。**命中 → 停手读完清单、落回执，再往下走**；**判不出类目（接手陌生项目、只有范围没有动作）→ 先按 §1.1「分诊与 skill 激活」探明类型，再回到本步读清单**；两者都不是才写「本轮无必读清单命中」。**这一步不许省略**
...
1. **工作区的规范文件 —— 最高优先级，必读**：用户全局 `~/.dsh/AGENTS.md`（或客户端等价文件）→ **当前工作区**从根到当前目录逐级的 `AGENTS.md` / `CLAUDE.md`，叠加层 `AGENTS.local.md` / `CLAUDE.local.md`。**这些文件里的约定优先级高于本文件**，冲突时以它为准。**「必读」不等于「必须有」**——逐级找完确实没有，如实说一句「本工作区无 `AGENTS.md`」就按下一项继续，**【不要】自作主张新建 `AGENTS.md` / `CLAUDE.md`**，除非用户明确要求。
   ...
   > **这一条与 §1.1 的分诊是同一件事的两面**：读规范文件既拿约定，也拿**项目类型的第一手证据**（技术栈、验证命令、禁区）——分诊从这里开始，读完顺手落成 §1.1 的三行分诊。
...
- **探索的第 4 样产出是分诊结论**：验证命令、目录地图、禁区三样齐了，项目类型基本也浮出来了——**顺手把它落成 §1.1 的三行分诊**，别让这次探索的结论随会话蒸发
...
- **两个例外要说话**：① **读取回执与三行分诊**（§1.1）是**给用户核对的执行痕迹**，不算旁白，照格式写出来；② 关键时刻各播报一句——发现意外、改变方向、遇到阻塞
...
- 声称完成前自查，逐条过：**本轮命中必读清单了吗、读了、落回执了吗**？**类目判不出时走分诊了吗、分诊完真读了吗**？代码真跑过吗？测试过了吗？清单都勾了吗？工作区收干净了吗？要交给用户的文件登记了吗？这轮的新结论写进记忆了吗？
...
- **提及了任务 → 直接开工**
  1. **先过硬规定**：扫 §1.1 十七类判断命中，**命中就停手读完**（超过 4 条取 4 条）并落下读取回执；**判不出类目就先分诊**（接手陌生项目最常见，见 §1.1「分诊与 skill 激活」），探明后回头读该类清单，**不许拿「判不出」当免门**；再读必读文件（§八）；先检索记忆（§九）。**发现指向失效立即上报**
...
| **MengTo/Skills** | 123 个技能：网页设计 81（GSAP / Three.js / 滚动叙事 / 材质与排版）、**网页游戏 20**（Three.js ARPG、关卡、敌人 AI、战斗、存档、性能、联调发布）、Codex 工作流 19、出图 2、UI 提示 1 | **网页游戏 / Three.js 交互**——十七类里只有这一家覆盖；也可按需取单个特效技能 |
...
**这一格分两半，别串用**：**做研究写论文**读 ①–④；**解题与核查解答**读 ⑤–⑦。两半的清单各自按「4 条及以内 → 全读」走。

### 一、做研究与写论文（①–④）

| skill | 定位 | 何时选它 |
|---|---|---|
| **academic-research-skills** | 论文全生命周期：research → write → review → revise → finalize，多智能体同行评审 | 从想法到投稿要**一条完整流水线** |
| **Supervisor-Skills** | 中文，博导经验炼成技能：Idea 构思 → 投稿 | **中文语境**、需要「导师视角」把关 |
| **scientific-agent-skills** | 165+ 技能 + 100+ 科学数据库，偏生物 / 化学 / 医学 / 材料 | 需要**跑具体学科的计算与数据库** |
| **AI-Research-SKILLs** | AI 研究工程：文献综述 → 想法 → 实验 → 写作 | **AI / ML 方向**的研究 |
...
| skill | 提供的方法 | 何时选它 |
|---|---|---|
| Anthropic 官方 **math-olympiad** | 审题解读检查 → 并行多路尝试 → **剥离思考痕迹** → 对抗式验证 → 非对称投票 → **敢说「没有把握」** | 要**证明**、要核查某个解答对不对、要反例；**需要高可信度时** |
| **求是Skill** | 实事求是总原则 + 九大方法论（矛盾分析、调查研究、实践认识论……） | 问题**复杂到抓不住重点**、需要先调查再判断时 |
| **cc-thinking-skills** | 28 个心智模型与批判性思维工具 | 需要**换角度**、做决策与权衡时 |
