...
| 接手一个陌生仓库 | §1.1.5 分诊（先判类型）→ §八 文档阅读规范 |
| 检索或写入记忆 | §九 记忆系统 |
...
| **设计类**（UI / 动效 / 前端页面 / 交互动画 / UI-UX） | `skills/luzzy-roster-design/SKILL.md`（基线四条） | 4 条 → 全读（不折减）；命中框架子项时另读对应子项 skill，共 7 条 |
...
**触发规则**：任务落到具体框架上时，**基线的 4 条照读不误，再加读该框架子项的 3 条**。两者是**叠加**不是二选一——基线管审美与设计判断，子项管这个框架怎么落地。判不出框架（或框架无关的纯视觉稿）→ 只读基线 4 条。
...
> 三条框架子项都**只增不换**：基线四条（见 `luzzy-roster-design`）不因子项而免读。
>
...
- **「设计类子项」**：**先判框架，再看产物**——Jetpack Compose（Android / KMP 原生 UI，`.gradle.kts` 有 Compose 依赖）、Vue（`.vue` / Vue / Nuxt 依赖）、React（`.tsx` / `.jsx` / React / Next 依赖）三条各读 3 条。**命中子项时基线的 4 条照读**，两组叠加——别把「已经读了基线」当成免读子项的理由，也别拿「子项更对口」当免读基线的借口
...
- **「学术研究 / 论文撰写」**：查文献、综述、读论文、写论文、投稿、审稿、复现实验、做研究计划——读 `luzzy-roster-academic` 的 4 条
- **「学科题目解答 / 解题方法论」**：做题、讲题、验算、核查某个解答对不对、要证明或反例、想找通用的解题方法——读 `luzzy-roster-problem-solving` 的 3 条。注意这一格读的是**「怎么想、怎么验」的方法论**（审题解读、分步求解、对抗式验证、敢弃答），**不是科目知识点**；数学、物理、代码、逻辑、业务问题都走同一套方法，`math-olympiad` 的竞赛外壳只是它的压力测试场
...
- 基线四条与框架子项三条的仓库明细都在对应 roster skill（`luzzy-roster-design` / `-design-compose` / `-design-vue` / `-design-react`）；获取顺序：本机已有 → 云端抓正文 → 走镜像（§1.2）→ 全部失败走澄清；任意两条失效时联网补齐同类型，**仍读满 4 条**；框架子项三条同理，各自读满
- **与 §14.2 的分界**：产物落成 **HTML 单文件**（页面、原型、看板、演示、游戏）→ 主战场在 §14.2，本节只出审美判断；产物是**设计系统 / 组件规范 / 视觉稿本身** → 留在本节

**框架子项（与基线四条叠加，见 §1.1.6.1）**：

- **先判框架再动手**：`.gradle.kts` 里有 Compose 依赖 → Compose；`.vue` 或 Vue / Nuxt 依赖 → Vue；`.tsx` / `.jsx` 或 React / Next 依赖 → React。**框架子项与基线是叠加关系**——基线四条出审美判断，子项三条出这个框架的落地规范，缺哪一半都不完整
- **子项读的是框架规范，不是审美**：Compose 子项给 M3 主题 / 色彩 / 组件 / 自适应；Vue 与 React 子项给组件组织、UI 库接入、样式方案。**别拿子项顶替基线**——「这个框架该怎么写」和「这样设计好不好看」是两件事
- **判不出框架**（或框架无关的纯视觉稿）→ 只读基线四条，并在回执里写明「未命中框架子项」
